...
Status: **11 frozen nodes — 6 `DRAFT_SORRY`, 3 `FROZEN`, 2 `SEALED`.**  The 5 sealed nodes are machine-checked and no
sealed node's axiom closure contains `sorryAx`.  Still open: `lem-boundary-routing`, `prop-circuit-iterate`, `prop-monotonicity`, `prop-passage`, `lem-decreasing-positions`, `prop-live-recurrence`,
