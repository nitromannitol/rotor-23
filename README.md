...
Status: **33 frozen nodes — 2 `CONDITIONAL`, 7 `DRAFT_SORRY`, 7 `FROZEN`, 17 `SEALED`.**  The 26 sealed nodes are machine-checked and no
sealed node's axiom closure contains `sorryAx`.  Still open: `prop-square-passage`, `lem-square-dual-path`, `lem-square-constrained-bonds`, `lem-square-exploration`, `lem-square-active-list`, `lem-square-forced-tests`, `prop-pendant-counterexample`,
