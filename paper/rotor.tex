...
\subjclass[2020]{Primary 60K35; Secondary 05C81, 82C20, 60G50, 82B43}
\keywords{Rotor walk, Eulerian walker, rotor-router, self-organized
	criticality, limit shape, recurrence, critical percolation}
...
\begin{document}
	...
	\smallskip
	The proofs in this paper have been formalized in the Lean~4 proof
	assistant, with six cited results taken as hypotheses: the abelian property
	\citep[Lemma~3.9]{HolroydLevineMeszarosPeresProppWilson2008},
	\citet[Lemmas~2.1 and~2.4]{FlorescuLevinePeres2016},
	\citet[Lemma~6]{HolroydPropp2010}, \citet[Theorem~1]{AngelHolroyd2012},
	the subadditive ergodic theorem \citep{Kingman1968}, and the domination
	theorem of \citet[Theorem~0.0]{LiggettSchonmannStacey1997}.  The
	formalization is available at \url{https://github.com/nitromannitol/rotor-23}.
	...
	Artificial intelligence tools were used in this work.  OpenAI's ChatGPT~5.6,
	through the Codex agent, and Anthropic's Claude proposed and checked proof
	steps, audited the arguments section by section, and edited the exposition
	and figures.  The Lean~4 formalization cited in
	Section~\ref{ssec:main-results} was written by ChatGPT~5.6, Claude, and
	GLM~5.3 agents under the authors' direction; it verifies every statement of
	Sections~\ref{sec:circuits}--\ref{sec:counterexample} from the six cited
	hypotheses listed there.  The authors checked every proof and are responsible
	for the mathematics.
	...
	Part~\textup{(iii)} does.  The rest of this subsection is devoted to its
	...
	\end{equation}
	Denote the law of the initial rotor at each vertex $x$ by $\nu_x$ and write $\P_0$ for the
	...
	Proposition~\ref{prop:live-recurrence}.  We also show that under the additional condition that the graph is doubly periodic and
	embedded in $\R^2$, a live path reaches distance $R$ from its first vertex with
	...
	We next show exponential decay under the extra hypothesis that $G$ is doubly periodic.
	...
	\begin{proof}[Proof of Proposition~\ref{prop:pendant-counterexample}]
		...
		zero, forcing at least one third of the rotors in $U$ not to point west.  For large $M$ a counting estimate rules this out:
		\[
		...
		\]
		Fix a rotor configuration in the complement of the event in the above display and suppose for the sake of contradiction that the induced walk returns to $o$.  Let $F$ be the directed edges traversed up to and including
